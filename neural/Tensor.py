...
class Tensor:
    def __init__(
        ...
        assert not (data is not None and shape is not None), (
            "Provide either data or shape, not both."
        )
        assert not (data is None and shape is None), (
            "Must provide either data or shape."
        )
        ...
        if data is not None:
            ...
            self.data = np.array(
                np.random.default_rng().random(shape), dtype=self.dtype
            )
    ...
    def __add__(self, other):
        ...
        def grad_fn(grad):
            if self.requred_grad:
                self._backward(grad)
            if other.requires_grad:
                other._backward(grad)
